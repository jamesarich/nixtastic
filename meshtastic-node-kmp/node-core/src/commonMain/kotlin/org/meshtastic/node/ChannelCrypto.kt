 ...
 * packets over the phone API. A client that participates in the mesh as a node has to do it itself,
 * because every mesh transport - LoRa, UDP multicast, BLE advertisement - carries `MeshPacket`s
 * whose payload is still `encrypted`.
 *
 ...
 * collisions are easy to find, so a matching hash is never authentication.
 */
public object ChannelCrypto {
     ...
     * semantics exactly (CryptoEngine / Channels.h):
     *
     *  - 0 bytes       -> no encryption; returns null and the packet is cleartext on air
     *  - 1 byte        -> index into the well-known default PSK. 0 is cleartext; 1 is the default
     *                     unchanged; 2..255 is the default with its last byte incremented by (n-1)
     *  - 16 / 32       -> a raw AES-128 / AES-256 key, used as-is
     *  - 2..15, 17..31 -> zero-padded to 16 or 32. A defensive fallback for malformed input, not
     ...
     *
     * Both ids are 32-bit on the wire but taken as [Long] here, because Kotlin's Int is signed and
     * a NodeNum above 0x7FFFFFFF is entirely ordinary.
     */
    ...
    public fun transform(payload: ByteArray, psk: ByteArray, packetId: Long, fromNode: Long): ByteArray {
        ...
        return aesCtrTransform(key, buildNonce(packetId, fromNode), payload)
    }
}
