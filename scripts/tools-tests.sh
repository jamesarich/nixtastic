...
"
# A description hand-written with its own machine tag must not get a second.
mk tagged-desc    "[darwin] already tagged"    reference "  machine: darwin
"
...
want='who-james-is some-feedback bench-serials tagged-desc alpha-project quoted-desc zeta-project'
...
grep -q '^- \[Tagged desc\](tagged-desc.md) - \[darwin\] already tagged$' "$idx" \
  || { echo "T19: machine tag rendered twice"; cat "$idx"; exit 1; }
