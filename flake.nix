    ...
    in
    {
      ...
      packages = forAllSystems (
        ...
        let
          ...
          #########################################################
          # Worktree outfitting, shared by .#sync and .#worktree.
          #
          # ensure_excludes: ignore the generated files via
          # .git/info/exclude (local, never committed) because editing a
          # tracked .gitignore in someone else's repo is not ours to do.
          # --path-format=absolute is load-bearing: `rev-parse -C <p>
          # --git-common-dir` answers RELATIVE to <p> (".git"), and using
          # that from the caller's cwd once put every pattern in the
          # WORKSPACE repo's exclude file instead (see 7388ecb).
          #
          # write_worktree_envrc: unlike the per-repo .envrc, this
          # hardcodes the workspace root — a worktree can sit anywhere,
          # including outside the workspace, so $(dirname "$PWD") cannot
          # reach it. printf, not a heredoc: this string is interpolated
          # into indented Nix strings and nixfmt reflows those, which a
          # heredoc terminator would not survive (same reason as
          # writeMcpJson above).
          #########################################################
          worktreeHelpers = ''
